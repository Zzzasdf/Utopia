...
namespace ConditionModule
{
    ...
    public class FuncOpenSelfControlReverseSubscriber : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            GameEntry.Instance.ConditionManager.Subscribe(eFuncOpenId.GetCondition(), SubscribeUpdate);
        }
        private void OnDestroy()
        {
            GameEntry.Instance.ConditionManager.Unsubscribe(eFuncOpenId.GetCondition(), SubscribeUpdate);
        }

        private bool Status() => GameEntry.Instance.ConditionManager.Status(eFuncOpenId.GetCondition());
